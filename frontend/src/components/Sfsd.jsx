...
function Sfsd() {
  ...
  const handleUpload = () => {
    ...
    axiosInstance.post('https://examportal-uii3.onrender.com/api/upload', formData)
      .then(res => { 
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axiosInstance.get('https://examportal-uii3.onrender.com/api/students');
        // Filter users belonging to "KKEM March DSA" batch
        const filteredUsers = response.data.filter(user => user.batch === 'KKEM March FSD');
  ...
  const sendEmails = async (email) => {
    try {
      await axiosInstance.post('https://examportal-uii3.onrender.com/api/email', { email });
